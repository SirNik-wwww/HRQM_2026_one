class_name ChaseKirtState extends EnemyMovementState
...
##_____________________________ ПЕРЕХОДЫ __________________________________##
	if PLAYER.is_dead == true:
		Trans.emit("StanKirtState")

	elif SELF.lives <= 0:
		Trans.emit("DeadKirtState")

	#elif dir.length() > 13:
		#Trans.emit("IdleKirtState")
...
##_______________________ передвижение ____________________________________##
	...
	if is_rand_dir_setted == false and walk_able == true:
...
func set_random_dir():
	...
	SELF.velocity.x = rand_dir.x * SELF._speed
	SELF.velocity.z = rand_dir.z * SELF._speed
